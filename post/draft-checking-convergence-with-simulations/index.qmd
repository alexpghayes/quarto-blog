...
- convergence rates from slopes in log-log plots, overlaying in ggplot from log-log regression
...
```{r}
#| fig-alt: "Line plot of Error versus Number of data points. The error decreases as the number of data points increases, following a curve that approaches zero but never reaches it."
library(ggplot2)
...
One thing that helps a lot here is to plot $\log \left(\left \Vert \hat \theta - \theta \right \Vert \right)$ as a function of $\log n$, and check if the resulting line has constant negative slope. The idea here is that if $\left \Vert \hat \theta - \theta \right \Vert \le C n^{-\alpha}$ with high probability, or whatever bound you prove, then $\log \left(\left \Vert \hat \theta - \theta \right \Vert \right) \le \log C - \alpha \log n$, i.e. a line with slope $-\alpha$ in $\log n$. So we can just check if things are linear after transformation, and this is easier to do visually.

```{r}
#| fig-alt: "The same Error versus Number of data points plot but on a log-log scale. The curve is now a perfectly straight line with a constant negative slope, making the rate of convergence much easier to identify."
ggplot() +
  aes(n, nice_error) +
  geom_line() +
  geom_point() +
...
```{r}
#| fig-alt: "Plot of noisy error estimates versus number of data points. The points bounce up and down significantly, making it difficult to see any underlying downward trend."
library(patchwork)
...
```{r}
#| fig-alt: "The noisy error estimates plot on a log-log scale with a linear regression line overlaid. Despite the noise, the downward linear trend is clearly visible, and the points cluster around the regression line."
...
- estimating and including parameters (via log-log OLS and also via power-law MLE -- see clauset2009)