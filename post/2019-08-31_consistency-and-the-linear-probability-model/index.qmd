...
A while back Twitter once again lost its collective mind and decided to rehash the logistic regression versus linear probability model debate for the umpteenth time. The genesis for this new round of chaos was @gomila_logistic_2019, a pre-print by Robin Gomila, a grad student in psychology at Princeton.
...
Suppose we have outcomes $Y \in \{0, 1\}$ and fixed covariate vectors $X$. The linear probability model is a *model*, that is, a set of probability distributions that might have produced our observed data. In particular, the linear probability model assumes that the data generating process looks like:
...
And this is the point where I got pretty confused, because the big question is: consistent under what model? Depending on who you ask, we could conceivably be assuming that the data comes from:
...
[^experimental_example]: In a lot of experimental settings that compare categorical treatments, you fit a fully saturated model that allocates a full parameter to the mean of each combination of treatments. $\hat \beta_\text{OLS}$ is consistent in this setting under both logistic regression and the LPM.
    ...
    Let's prove this for logistic regression first. Suppose we have groups $1, ..., k$ and the data comes from a logistic regression model. We represent the groups in the model via one hot coding, omitting an intercept for identifiability. That is, we assume
...
#| message: false
#| warning: false
#| fig-alt: "Comparison of expectations for three models: binomial GLM (S-shaped curve), Gaussian GLM (linear diagonal), and linear probability model (linear diagonal clipped at 0 and 1). All three are similar in the range where probabilities are between 0.2 and 0.8."
library(tidyverse)

labels <- c("binomial GLM (linear rescaling)", "Gaussian GLM", "linear probability model")
...
tibble(
  ...
  geom_line(size = 1) +
  scale_color_viridis_d(begin = 0.15, end = 0.85, labels = labels) +
...
It only took several weeks of misreading @boos_essential_2013 and asking dumb questions on Twitter to figure this out. Thanks to Achim Zeileis, James E. Pustejovsky, and Cyrus Samii for answering those questions. Note that @white_heteroskedasticity-consistent_1980 is another canonical paper on robust standard errors, but it doesn't click for me like the M-estimation framework does.
...
2. If there are multiple reasonable estimators, fit them all. If they give substantively different conclusions, don't just pick your favorite for the write-up. That's usually a sign your data can't distinguish between the models you're considering, so it's worth digging into why before you trust any of them.
...
[^nonlinearity]: Gomila also makes an argument against using $\hat \beta_\text{logistic MLE}$ based on some papers that show it handles interactions poorly. This sort of misses the forest for the trees in my opinion. GLMs impose a specific form (linear, or linear on the link scale) on the conditional expectation, and that's often wrong in some part of the covariate space. And maybe linear main effects won't kill you, but modeling interactions with linear terms amounts to fitting global hyperplanes for interactions. This is undesirable because interactions are almost always local, not global. If you want to model interactions carefully, you need something flexible like a GAM or kernels or whatnot.
