...
```{r}
#| fig-alt: "Localization statistics plot showing cumulative curves for row and column sums of the cocktail-ingredient incidence matrix. Vertical lines indicate average row and column sums."
...
```{r}
#| fig-alt: "Mixed visualization showing IPR pairs, mixing matrix, and varimax factor pairs for both rows (cocktails) and columns (ingredients)."
plot_ipr_pairs(fa)
...
```{r}
#| fig-alt: "Scree plot showing the eigenvalues or singular values of the cocktail data matrix, used to determine the appropriate rank for factor analysis."
screeplot(fa)
```
...
```{r}
#| fig-alt: "Sparsity plot (spy plot) of the Z matrix, showing that individual drinks typically only have a few main ingredients."
# Z is sparse: drinks typically only load on a few ingredients at a time
image(fa$Z)
```

```{r}
#| fig-alt: "Mixing matrix plot showing the relationships between cocktail factors and ingredient factors, with custom names for the factors."
z_names <- fa |> 
  get_z_hubs(1) |> 
  pull(id)
...
```{r}
#| fig-alt: "Bar chart of ingredient loadings for cocktail factors. PC1 highlights the contrast between powdered sugar/egg/gin and simple syrup/lime/tequila."
fa |> 
  get_y_hubs(3) |> 
...
```{r}
#| fig-alt: "Bar chart of cocktail loadings for factors, showing which specific drinks most strongly represent each component."
fa |> 
  get_z_hubs(5) |> 
  mutate(id = reorder_within(id, loading, factor)) %>%
  ggplot(aes(loading, id, fill = loading > 0)) +
...
```{r}
#| fig-alt: "IPR pairs and mixing matrix for the scaled cocktail data model."
library(Matrix)
