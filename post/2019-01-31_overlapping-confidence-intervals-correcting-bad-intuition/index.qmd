...
subtitle: |
  some math to show that confidence intervals of significantly different parameters can overlap
date: "2019-01-31"
categories: [math stat, methods]
execute:
  echo: false
  message: false
  warning: false
...
```{r}
#| fig-alt: "A dot-and-whisker plot showing regression coefficients for various predictors of miles per gallon (mpg) in the mtcars dataset. Most coefficients have confidence intervals that overlap with each other, though some like 'wt' (weight) are clearly distinct from zero."
library(tidyverse)
...
[^covariance]: This is actually a bad example because OLS estimates for regression coefficients are correlated. A better example would be plotting confidence intervals for independent group means like you get from classic cross-sectional experimental designs.

```{r}
#| fig-alt: "A bar chart showing the same regression coefficients as the previous plot, but using bars for estimates and error bars for confidence intervals. The visualization emphasizes the overlap between intervals for different terms."
ggplot(td, aes(term, estimate)) +
  geom_bar(stat = "identity") +
  geom_errorbar(aes(x = term, ymin = conf.low, ymax = conf.high))
```

and do something similar. While this procedure is intuitively satisfying, you should avoid it because it doesn't work.
...
In practice, though, the $\sqrt 2$ is still there, and we have this expression $2 \cdot (1 - \Phi(\sqrt 2 \cdot \color{purple}{z_{1 - \alpha / 2}} ))$, which is a bit hard to parse. To get an idea of what this looks like, I've plotted $\alpha$ (used to construct the confidence intervals for the mean) against the actual type I error:

```{r}
#| fig-alt: "Line chart comparing the desired type I error rate (alpha) on the x-axis to the true type I error rate on the y-axis. A dashed identity line shows that the true error rate is always lower than the desired one, indicating that the confidence interval overlap heuristic is conservative."
tibble(
  alpha = seq(0.00001, 0.9999, length.out = 200),
  type_i_error = 2 * (1 - pnorm(sqrt(2) * qnorm(1 - alpha / 2)))
) %>%
  ggplot(aes(alpha, type_i_error)) +
  geom_line(size = 1) +
  geom_abline(size = 1, slope = 1, color = "steelblue", linetype = "dashed") +
  labs(
    title = "True versus desired type I error rate",
    subtitle = "Blue line is the identity",
    y = "True type I error rate",
    x = "Desired type I error rate (alpha)"
  )
```
...
Remember that we found the variance of $\color{blue}{\bar y}  - \color{blue}{\bar x}$ using the rule $\mathbb{V}(a \cdot X + b \cdot Y) = a^2 \cdot \mathbb{V}(X) + b^2 \cdot \mathbb{V}(Y)$ and it was $2 \cdot \color{green} {\sigma^2 \over n}$. To get the standard error, we take the square root and get $\sqrt 2 \cdot \color{green} {\sigma \over \sqrt n}$.

But if we had accidentally worked on the standard deviation scale instead, and mistakenly assumed
...
In future posts I plan to build off this idea and explore statistics as a primarily sociological problem.
