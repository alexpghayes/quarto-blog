...
and Tesla stock started tanking. I find this absolutely hilarious, especially since he did this a while back and got fined like several million dollars for tinkering with the market or something illegal like that.

Anyway, I asked myself: can we causally attribute Tesla's tanking stock price to this tweet?
...
I used the S&P500 as my synthetic control and pulled 1-minute resolution ticker data using Tiingo with the `riingo` package, then blindly shoved the data into `CausalImpact`. We get the following:

```{r}
#| message: false
#| warning: false
#| fig-alt: "Three-panel CausalImpact plot showing the impact of Elon Musk's tweet on Tesla stock price. The top panel compares the actual Tesla stock price (black line) with its counterfactual prediction (blue line with 95% interval) based on the S&P 500. After the tweet (indicated by a vertical dashed line), the actual price drops significantly below the predicted price. The second panel shows the pointwise causal effect, and the third panel shows the cumulative causal effect, estimated at approximately -$40 per share."
library(tidyverse)
library(zoo)
...
data_path <- here::here(
  "post",
  "2020-05-01_elon-musk-send-tweet",
  "telsa-sp500-prices.rds"
)
...
Of course, you probably shouldn't believe these estimates since I didn't generate them with a neural net.
