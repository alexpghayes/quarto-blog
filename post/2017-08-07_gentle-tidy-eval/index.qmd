...
Sometimes it is nice to write functions that accept non-standard inputs, like `dplyr` verbs. For example, we might want to write a function with the same effect as
...
To do this we need to capture our input in `quosures` with `quo` and `quos` when programming interactively.
...
Now our function call is non-standard. Note that `quos` can capture an arbitrary number of arguments, like we have here. So both of the following calls are valid:
