...
[^1]: The other nice thing about Stan is that you end up using the same estimator for all your models. This means there's no context switching when you move to a different estimator and have to remember how to do inference all over again.
...
[^2]: After reading [What nobody tells you about documentation][types_of_doc], I'd classify most existing Stan resources as Reference and How To Guides. I think there's a lot of opportunity to introduce the language to a broader audience via more introductory tutorials.
...
Integers can be promoted to reals, but reals are never demoted to integers. The other two objects you should know about are `vector`s and `matrix`s. `vector`s and `matrix`s contain `real` elements. When you declare `vector`s and `matrix`s, you have to tell Stan how big they are:
...
Note that the warnings go away! The new model actually makes sense, and as a consequence no longer stretches the sampler to a breaking point. At this point I was happy with the core logic of the model, so I used ShinyStan to check MCMC diagnostics and make sure that all the chains were mixing, etc:
...
I wanted $P(y = 0)$ given a person's set of responses $R_{i1}, ..., R_{iA}$. This was one of those instances where I was intimidated by how to figure that out, but then it all became clear with a minute or two of pen and paper work.
...
```{r}
#| fig-alt: "Density plots of estimated response probabilities for each of 15 questions across 5 response categories, faceted by question and response, comparing Group 0 and Group 1."
library(ggplot2)
...
In terms of more didactic material, Michael Clark's [Bayesian Basics][mclark] is a nice introduction to Bayes. You may also enjoy Rasmus Bååth's [exercises][farmer_jon], which are an introduction to both Stan and Bayesian inference.
...
- [Stan Manual][stan_manual]: The canonical reference. Your answer is probably somewhere in here, if you can find it.
