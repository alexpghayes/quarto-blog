...
![](bootstrap_world.png){fig-alt="Diagram explaining the logic of the bootstrap: given an observed sample, generate bootstrap samples by sampling from the empirical distribution of the observed sample."}
...
data <- read_xlsx(here(
  "post",
  "2018-05-03_performance-assessments-via-bootstrap-variants",
  "us_avg_tuition.xlsx"
)) %>%
  transmute(
    state = State,
...
```{r}
#| fig-alt: "Scatterplot of average college tuition in 2014-2015 on the x axis versus average college tuition in 2015-2016 on the y axis, with an overlaid regression line. The two tuitions are very close to perfectly linearly related."
ggplot(data, aes(avg_tuition_14_15, avg_tuition_15_16)) +
  geom_smooth(method = "lm", se = FALSE, formula = 'y ~ x') +
  geom_point(size = 2) +
  labs(
    title = "Average College Tuition",
...
There are several ways to calculate the error on our bootstrap samples. One first step is to calculate the *bootstrap in sample error* by estimating the performance of a model fit on a bootstrap sample, on that same bootstrap sample $X^*_b$ itself.
...
However, we also know that for a given bootstrap sample, some of the original data didn't get used to fit $f_b$. We can use that data to calculate *bootstrap out of sample error*:
...
The idea is that the 0.632 estimator can be optimistic, so the 0.632+ estimator takes overfitting into account to correct this. If the relative overfitting is zero, the 0.632+ estimator reduces to the 0.632 estimator.
...
```{r}
#| fig-alt: "Boxplots overlaid with jittered actual values of performance metrics for each of the bootstrap approaches. All five bootstrap approaches produce similar estimates of the root mean squared prediction error."
clean_performance %>%
...
For this particular model, we see that the 0.632 and 0.632+ estimators are pretty much the same. This makes sense because a linear model fits the data quite well and so there should be little overfitting. We can also see that the bootstrap in sample error rate has the lowest median.
...
As a rule of thumb from simulations in specific settings (see the footnote), the optimism bootstrap with $B = 200$ to $B = 400$ tends to work well below about 20,000 data points, with cross-validation[^4] or simple data splitting becoming more reliable past that. Any steps taken to develop a model should be performed on each bootstrap dataset. For example, if you use LASSO for feature selection and then fit a model, the feature selection step needs to be included in the bootstrap procedure.

[^4]: In the simulations in @steyerberg2001, cross validation was much less stable than the bootstrap on data sets with fewer than 20,000 data points. Frank Harrell has run similar simulations suggesting you need on the order of 100 repetitions of 10-fold cross-validation to get accurate error estimates at that data size (@harrell2015).

In some studies (@steyerberg2001) the 0.632 and 0.632+ estimators have similar performance. Asymptotically, the 0.632 estimator is equivalent to the optimism bootstrap (@efron1994)[^5]. It isn't clear if one is preferable over the other in finite samples, so I'd just stick with the estimator your audience is most likely to already be familiar with. You should be fine so long as you avoid using the *bootstrap in sample error* estimate.
