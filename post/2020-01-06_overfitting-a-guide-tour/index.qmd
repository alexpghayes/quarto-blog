...
mistakenly confuse random, apparent structure for true, systematic
...
```{r}
#| fig-alt: "Scatter plot of 20 simulated data points showing a clear negative linear relationship. A dashed grey line represents the true conditional expectation."
library(tidyverse)
...
```{r}
#| fig-alt: "Degree 7 polynomial fit shown as a maroon line. The line wiggles excessively to pass through training data points, deviating significantly from the true linear expectation (grey dashed line)."
library(broom)
...
```{r}
#| fig-alt: "Simple linear regression fit shown as a green line. The line closely follows the true linear expectation (grey dashed line) and does not overfit to the random noise in the data."
...
```{r}
#| fig-alt: "Line plot of R-squared versus polynomial degree. As the degree of the polynomial increases from 1 to 17, the R-squared value steadily increases towards 1, suggesting a better fit even though the true model is degree 1."
...
fits %>%
  ...
  labs(
    title = expression(paste(
      "High degree polynomials fit better according to ",
      R^2
    )),
    y = expression(R^2),
    x = "Degree",
...
```{r}
#| fig-alt: "Faceted scatter plots with polynomial fits of degrees 2, 4, 7, 11, 14, and 17. As the degree increases, the maroon prediction lines become increasingly wiggly, conforming more to the random noise in the data points and moving further away from the true dashed grey line."
fits %>%
...
[^2]: See @piantadosi_one_2018 for an example of a model with a single
    parameter that can nonetheless perfectly interpolate any finite
...
```{r}
#| fig-alt: "Scatter plot showing bivariate data points clearly separated into two elliptical clusters, labeled A and B, with different colors and shapes."
cluster_data %>% 
  as_tibble() %>% 
...
```{r}
#| fig-alt: "Estimated clusters using a 5-component Gaussian Mixture Model. The plot shows the original two clusters being incorrectly split into five overlapping ellipses, demonstrating overfitting."
library(mclust)
library(factoextra)
...
systematic structure in noisy, observed conditional mean of $Y$ given
...
[^3]: Random variation in the test set will cause irreducible error that
    we cannot make disappear. More often than we would like to admit,
    the irreducible error is large.

It turns out that we can study this phenomenon more formally. The math is
...
\end{align}
$$
This relationship holds for most important loss functions. It tells us that test error, or generalization error, is almost always
...
We'll discuss the implications of the train-test error relationship more in
...
[^4]: There is also the bizarre scenario when an estimator produces
    ...
    If you observe this phenomenon on an applied problem, I recommend
...
to get a new "unbiased estimate with smaller variance". In practice, the
...
[^6]: The risk estimates across the $k$ folds are both biased and
    ...
    results in a better estimate. Second, the held-out risk estimates
...
@shao_linear_1993 proves that cross validation isn't consistent for
...
particular discussing how cross-validation procedures differ when the
