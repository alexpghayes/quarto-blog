...
```{r}
#| fig-alt: "Line chart titled 'Riegel curve based on 5K in 18:52' showing the black line of maximal maintainable speed for a given distance. The chart is overlaid on colored rectangles representing different exercise intensity zones: recovery (lightest), endurance, tempo, threshold, VO2 max, and anaerobic (darkest)."
library(tidyverse)
library(here)
library(lubridate)
library(mgcv)
...
[^3]: I stopped running in April after being prescribed Cipro for an infection. Cipro and other fluoroquinolones have the side effect of [*spontaneous tendon rupture*](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC4080593/pdf/i1062-6050-49-3-422.pdf) for up to 6 months after use, although most of the risk is in the first month after usage. Achilles tendons are at the highest risk. If you run, ask to be put on another medication!

```{r}
#| fig-cap: "Run in black."
#| fig-alt: "Map showing the path of a single run in black on a watercolor-style map of a neighborhood."
...
run_map <- function(run_df) {
  register_stadiamaps("3d423846-01ad-4275-a087-0e2301fc56bb")
  ...
  get_stadiamap(run_bbox, zoom = 15, maptype = "stamen_watercolor") %>% 
    ggmap() + 
    geom_path(
      data = run_df,
      aes(lon, lat),
...
```{r}
#| fig-alt: "Line chart titled 'Riegel 5K Equivalent Running Times' showing individual run data points and a GAM-estimated mean line in steelblue with a 95% confidence interval shaded in grey. The chart shows Riegel 5K time in minutes over time, with a rug plot on the x-axis indicating the timing of runs."
...
```{r}
#| fig-alt: "Line chart titled 'Instantaneous change in Riegel 5K time' showing the estimated first derivative of the GAM fit. The y-axis shows the change in mean Riegel 5K time (minutes/day). A horizontal dashed line at zero separates fitness improvements (below zero) from fitness losses (above zero). The estimated curve and its simultaneous 95% confidence interval show that the change is never statistically significant."
