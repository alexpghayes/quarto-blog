...
There are only $K-1$ coefficient vectors in order to prevent overparameterization[^1]. The purpose here isn't to describe the model in any meaningful detail, but rather to remind you of what it looks like. I strongly encourage you to read [this fantastic derivation](http://data.princeton.edu/wws509/notes/c6s3.html) of multinomial logistic regression, which follows the work that led to McFadden's Nobel prize in economics in 2000.

[^1]: Some machine learning courses present multinomial regression using a $K \times p$ coefficient matrix, but then estimate the coefficients with some sort of penalty (e.g. ridge). This parameterization is overparameterized: replacing every $\beta_k$ with $\beta_k + \gamma$ for any fixed vector $\gamma \in \mathbb R^p$ leaves all the class probabilities, and hence the likelihood, exactly unchanged. So the likelihood is flat (not identified) along this direction and there's no unique MLE unless you break the symmetry, e.g. with a penalty that picks out a particular point along the ridge. Statisticians more commonly sidestep the issue entirely by fixing one class's coefficients to zero and estimating the $(K-1) \times p$ parameterization directly, which is identified and has a unique MLE.
...
This says: hold $x$ constant for the variables of interest and take the average prediction over all combinations of the other variables in the training set. So we need to pick variables of interest, and also to pick a region of the space that $x_s$ lives in that we are interested in. Be careful extrapolating the marginal mean of $f(x)$ outside of this region!
...
```{r}
#| fig-alt: "Partial dependence plot for Sepal.Length showing estimated marginal probabilities for each of three iris species. Setosa has a high probability for small sepal lengths, while versicolor and virginica have higher probabilities for larger sepal lengths."
pd %>%
  ggplot(aes(!!var, marginal_prob, color = class)) +
  geom_line(size = 1) +
  scale_color_viridis_d() +
  labs(
    title = paste("Partial dependence plot for", quo_name(var)),
...
```{r}
#| fig-alt: "Partial dependence plots for all four iris features. Sepal.Length and Sepal.Width show relatively flat marginal probabilities. In contrast, Petal.Length and Petal.Width show strong separation, with setosa associated with low values, versicolor with middle values, and virginica with high values."
all_dependencies %>%
  ...
  scale_color_viridis_d() +
  labs(
    title = "Partial dependence plots for all features",
