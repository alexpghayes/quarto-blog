...
- **Correctness tests** check whether the code calculates the quantity it is supposed to calculate.
...
Correctness tests are by far the most important tests. Correctness tests come in two flavors: unit tests and integration tests. In the modeling context, a *unit test* checks the correctness of an individual calculation in a fitting procedure. An *integration test* means shoving data into the fitting procedure and making sure that the resulting estimates are correct.
...
The gold standard is to compare to a reference implementation. With research code, which is often the first implementation of a method, this might not be possible. Additionally, it's pretty rare to be able to work out the solution by hand.
...
![](sfpca_algorithm_1.png){fig-alt="Pseudo-code for the SFPCA algorithm."}

They used a MATLAB reference implementation available online^[It has since been replaced with a more fully-fledged R package [`MoMa`](https://github.com/DataSlingers/MoMA).], which looked like:

[sfpca_reference_implementation]: http://www.stat.rice.edu/~gallen/software/sfpca/

![](sfpca_ref_impl.png){fig-alt="Screencap of the SFPCA reference implementation in MATLAB."}

This is a good reference implementation because the pseudocode is close enough to the actual code that we can be confident that the translation is correct almost by eyeballing it. I ended up [implementing][gsoc_repo] this algorithm in both R and C++ and testing against this reference implementation. Curious parties can read the correctness test [here][gsoc_test].
...
Note that my test is really an integration test, making sure the entire computation is correct. I don't do any tests on sub-functions or calculations, although it would be good to write a unit test for the soft thresholding function.
...
![](adaptive_impute.png){fig-alt="Screenshot of pseudocode for the AdaptiveImpute matrix completion algorithm."}
...
Convergence tests check that iterative processes have actually reached solutions. Ideally you want packages to confirm that they converge on a variety of sample problems. You also want them to perform runtime convergence tests.
...
We can contrast this behavior with `lme4`, which explicitly [warns users](https://github.com/lme4/lme4/blob/366375f3d599fe27df250f0216d6dbca472d2caf/R/checkConv.R#L68) about convergence failures. The package also [tests](https://github.com/lme4/lme4/blob/master/tests/testthat/test-catch.R#L9) to make sure it issues this warning.

It can often be difficult to test for convergence, but for simple maximum likelihood or convex problems, I'd love to see more of these tests. I suspect there are a lot of convergence failures out there that we don't know about just because we haven't checked.

Other communities have taken different approaches. For example, the Stan community is careful to emphasize the importance of MCMC convergence checks as part of the modeling workflow, a task that needs to be repeated by the data analyst for each model they fit. The machine learning community sometimes deals with convergence via early stopping, which is more a sort of statistical convergence than algorithmic convergence.
...
So far I've talked about things to test at a fairly high level. Now I'd like to switch gears and talk about very specific issues that you need to be careful about, especially in R.
...
Here `conf.levl = 0.9` should read `conf.level = 0.9`, so the code has silently gone ahead and calculated a 95 percent confidence interval instead of a 90 percent confidence interval. This issue appears over and over in modeling code, and you can read more about it in the [tidyverse principles][inspect_the_dots] book draft.
...
This is mostly because packages that wrap lots of other packages (e.g. `broom`, `parsnip`, `mlr`, etc, etc) don't normally do integration tests to make sure that the wrapped estimates are correct, and so things slip through the cracks every once in a while.
...
- No tests, or tests don't substantively check correctness.
...
I agree with all of this. Additionally, I'd pretty strongly endorse packages that have been reviewed by [ROpenSci](https://ropensci.org/). I'm less certain how I feel about packages published in the [Journal of Open Source Software](https://joss.theoj.org/) (JOSS) and the [Journal of Statistical Software](https://www.jstatsoft.org/index) (JSS). More peer review is always better than less, but my impression is that peer review does a lot more to ensure that package authors follow good software engineering practices than it does to ensure actual correctness.
