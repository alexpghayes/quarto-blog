...
```{r}
#| fig-alt: "Scatter plot of self-assessed IQ (SAIQ) versus actual IQ. A dashed identity line shows that most points fall above the line, indicating general overestimation of IQ across all levels."
data |> 
  ggplot(aes(IQ, SAIQ)) +
...
```{r}
#| fig-alt: "Scatter plot of overconfidence (SAIQ minus IQ) versus IQ. A downward-sloping trend line shows that as IQ increases, the level of overconfidence decreases."
data |> 
...
```{r}
#| fig-alt: "Plot showing estimated sigma (variability) of self-assessed IQ as a function of IQ. A ribbon shows the uncertainty around the estimate, with actual squared residuals overlaid as points."
data |> 
...
```{r}
#| fig-alt: "Fitted plot from the gamlss model showing the conditional distribution of SAIQ as a function of IQ."
...
```{r}
#| fig-alt: "A diagram showing traditional Dunning-Kruger plots in panels A and B, which dichotomize performance, and a more appropriate scatter plot in panel C."
...
```{r}
#| fig-alt: "Two-panel plot visualizing the Dunning-Kruger effect from regression. The left panel shows that high IQ individuals overestimate their IQ by less than low IQ individuals. The right panel shows the fitted overconfidence decreasing as IQ increases."
library(patchwork)
