...
# https://osf.io/dg547/
raw <- read_csv(here("post", "xxxx-xx-xx_dunning-kruger", "Dunning-Kruger_INTELL_UPLOAD.csv"))
View(raw)

raw |> 
  select(contains("IQ"))

fit <- lm(SAI_winsorized_IQ ~ Raven_IQ, data = raw)
fit
...
```{r}
raw |> 
  ggplot(aes(Raven_IQ, SAI_winsorized_IQ)) +
  geom_point() +
  geom_abline(slope = 1, linetype = "dashed", color = "darkgrey") +
  theme_minimal()

raw |> 
  ggplot(aes(Raven_IQ, SAIQ_IQ_dif)) +
  geom_point() +
  geom_smooth() +
  geom_hline(yintercept = 0, linetype = "dashed", color = "darkgrey") +
  labs(
    y = "Self-assessed IQ - Measured IQ (i.e. overconfidence)",
    x = "Measured IQ"
  ) +
  theme_minimal(16)

fit <- estimatr::lm_robust(SAI_winsorized_IQ ~ Raven_IQ, data = raw)
broom::tidy(fit, conf.int = TRUE)
```
...
raw_no_na <- raw |> 
  select(SAI_winsorized_IQ, Raven_IQ) |> 
  na.omit()

# https://stats.stackexchange.com/questions/508818/how-to-model-conditional-variance
model <- gamlss(SAI_winsorized_IQ ~ Raven_IQ, 
                sigma.formula = ~ Raven_IQ, 
                family = NO2, 
                data = raw_no_na)
