...
**2020-01-05 6:42 pm CST**: Updated post to keep up with developments in the `tidymodels` ecosystem.
...
## Warning: development code in use!

This post relies on the development versions of `tune`, `parsnip` and `modeldata`, as well as the new `tidyr` [pivoting](https://tidyr.tidyverse.org/dev/articles/pivot.html) functionality, so make sure you have the latest version of `tidyr` from CRAN. Note that `tidymodels` packages are undergoing rapid development and are not particularly stable. Writing this post has taken much longer than originally anticipated because I keep coming [across](https://github.com/tidymodels/parsnip/issues/251) [bugs](https://github.com/tidymodels/parsnip/issues/172).

In any case, you should be able to install everything you need with:
...
Now we can wrap up the first step and fit each of these trees on the full dataset. Here I use [`furrr::future_map()`](https://davisvaughan.github.io/furrr/) to do this in parallel, taking advantage of the embarrassingly parallel nature of model fitting.
